	...
	void snakeSteering(KeyEvent keyEvent) {
		...
		if ((key == KeyCode.UP) || (key == KeyCode.RIGHT) || (key == KeyCode.DOWN) || (key == KeyCode.LEFT)) {
			...
			if (model.gameOver == true) {
				NetworkController.setHighscore(namePlayer1.getText(), point_counter_player1); // set highscore on server
																								// database
				...
				// DatabaseController.setSpielstand(player1_id, player2_id,
				// point_counter_player1, point_counter_player2, formatDateTimeStart,
				// formatDateTime, gameDuration); // local
				NetworkController.setGamedata(player1_id, player2_id, point_counter_player1, point_counter_player2,
						formatDateTimeStart, formatDateTime, gameDuration); // network
				timer.stop();
				timerPlayerTwo.stop();
				gameSelection();
			}
...
//			modelPlayerTwo.fruitBounds = foodImage1.getBoundsInParent(); // Player Two
			...
			if (modelPlayerTwo.gameOverPlayerTwo == true) {
				NetworkController.setHighscore(namePlayer2.getText(), point_counter_player2); // set highscore on server
																								// database
				gameEnd = LocalDateTime.now();
				DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd-HH-mm-ss");
				String formatDateTime = gameEnd.format(formatter);

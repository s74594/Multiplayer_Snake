...
public class NetworkController {
	...
	/**
	 * requests password for given user from server and compare it with user entered
	 * password
	 *
	 * @param name
	 * @param password
		...
		try {
			while ((messageIn = SocketClient.reader.readLine()) != null) {
				...
				if ((messageInJSON.has("sql_highscore_table_answer"))) {
					...
					highscoreTable = new Gson().fromJson(messageInJSON.getString("sql_highscore_table_answer"),
							listType);
	 ...
	 * @param duration
	 */
	public static boolean setGamedata(String player1id, String player2id, int player1_points, int player2_points,
			String startTime, String datenow, int duration) {
		// send message
		JSONObject messageOutJSON = new JSONObject();
	...
	public void multiplayerSnakeStatus(String name, String points, String eatFruit, String gameOver, String snakeX,
			String snakeY) {
